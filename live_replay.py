...
# Columns needed from the parquet
_COLS = ["user", "day", "if_anomaly_score", "if_percentile_rank", "if_risk_band"]
...
def _load_records() -> list[dict]:
    ...
    if os.path.exists(_LOCAL_MERGED):
        df = pd.read_parquet(_LOCAL_MERGED, columns=_COLS)
        ...
        try:
            ...
            df = pd.read_parquet(cached, columns=_COLS)
    ...
    for idx, row in df.iterrows():
        records.append({
            ...
            "_score_ms":        0.0,   # no live scoring; dashboard ignores this field
            "event_index":      int(idx),
        })
