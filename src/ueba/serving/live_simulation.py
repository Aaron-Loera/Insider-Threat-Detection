...
# Paths (loaded from ueba.config — paths.local.py overrides still apply)
...
# Ordered model feature columns persisted alongside the scaler at training time
# (see Autoencoder.ipynb). This is the authoritative train/serve column contract.
FEATURE_COLS_PATH           = os.path.join(os.path.dirname(SCALER_PATH), "feature_cols.json")
...
class LiveScorer:
    ...
    def score_row(self, row_df: pd.DataFrame) -> dict:
        ...
        # presence and order); otherwise drop the known non-feature columns
        # (config.NON_FEATURE_COLS) plus any unnamed parquet index columns.
        if self.feature_cols is not None:
            feat_df = row_df.reindex(columns=self.feature_cols)
        else:
            drop_cols = [c for c in row_df.columns
                         if c in config.NON_FEATURE_COLS or str(c).startswith("Unnamed:")]
