...
from ueba.config import HF_ORG as _HF_ORG
...
def _build_manifest(V: str) -> list[tuple[str, str, bool, str]]:
    ...
    # Per-user explainability details generated by Reconstruction_Error_Explainer.ipynb.
    # Each file is ~46 KB; the folder may not exist yet — entries are always optional.
    _details_dir = os.path.join(
        BASE_DIR, "explainability", "alert_table", f"alert_table_{V}", "details"
    )
    _detail_entries = [
        (f, f"details/{os.path.basename(f)}", False, _DATASET)
        for f in sorted(_glob.glob(os.path.join(_details_dir, "*.parquet")))
    ]
    ...
    return [
        ...
        # Per-user explainability details (details/{user}.parquet)
        *_detail_entries,
...
def _run(args: argparse.Namespace) -> None:
    V            = args.version
    dataset_repo = args.dataset_repo or f"{_HF_ORG}/ueba-v{V}"
    model_repo   = args.model_repo   or f"{_HF_ORG}/ueba-models-v{V}"
